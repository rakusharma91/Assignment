...
app.post('/api/get-states',authenticateRequest, async (req, res, next)=>{
	...
	if(fileData && fileData.length){
		fileData.forEach(records => {
			...
			if(records["Customer ID"] && records["Customer Name"]  && !customerList[records["Customer ID"]]) customerList[records["Customer ID"]] = {value: records["Customer ID"], label: records["Customer Name"]+" ("+records["Customer ID"]+")" };
		});
	}
	let fList = Object.values(stateList);
	let fCList = Object.values(customerList);
	sendApiResponse(req, res, next, {
		status		:	STATUS_SUCCESS,
		result		:	fList && fList.length && sortArray(fList, "value") || [],
		customers	:	fCList && fCList.length && sortArray(fCList, "value") || []
	});
});
...
app.post('/api/get-state-stats',authenticateRequest, async (req, res, next)=>{
	...
	if(fileData && fileData.length){
		fileData.forEach(records => {
			if(records.State && records.State == stateName){
				...
				if(odDate){
					...
					if(endDate && odDate > endDate) isValid = false;
				}

				/** Add Customer Validation */
				if(customerId && customerId != records["Customer ID"]) isValid = false;
		...
		if(stats.subcategory && stats.subcategory.length) stats.subcategory = stats.subcategory.slice(0, 7)
		if(stats.productname && stats.productname.length) stats.productname = stats.productname.slice(0, 6)
